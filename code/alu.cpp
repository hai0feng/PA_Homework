...
void p_add(LweSample* sum, const LweSample* a, const LweSample* b, const TFheGateBootstrappingCloudKeySet* ck, const size_t size) {
  LweSample *carry = new_gate_bootstrapping_ciphertext(ck->params),
            *tmp_s = new_gate_bootstrapping_ciphertext(ck->params),
            *tmp_c = new_gate_bootstrapping_ciphertext(ck->params);
...
void stone_add(LweSample* sum, const LweSample* a, const LweSample* b, const TFheGateBootstrappingSecretKeySet *keyset,const TFheGateBootstrappingCloudKeySet* ck, const size_t size) {
    	const LweParams *in_out_params = keyset->params->in_out_params;
    	// carries
    	LweSample *Q1 = new_LweSample_array(size, in_out_params);
	LweSample *Q2 = new_LweSample_array(size, in_out_params);
	LweSample *p = new_LweSample_array(size, in_out_params);
	LweSample *temp = new_LweSample_array(size, in_out_params);

	#pragma omp parallel for num_threads(NUM_THREADS)
  	for(int i = 0; i < size; i++) {
      		bootsXOR(&p[i], &a[i], &b[i], ck);
      		bootsAND(&Q1[i], &a[i], &b[i], ck);
    	}

	#pragma omp parallel for num_threads(NUM_THREADS)
	for(int i = 0; i < size; i++) {
		if(i-1>=0) {
			bootsAND(&temp[i], &p[i], &Q1[i-1], ck);
			bootsXOR(&Q2[i], &Q1[i], &temp[i], ck);
		}
		else
			bootsCOPY(&Q2[i], &Q1[i], ck);
	}

	#pragma omp parallel for num_threads(NUM_THREADS)
	for(int i = 0; i < size; i++) {
		if(i-2>=0) {
			bootsAND(&temp[i], &p[i], &p[i-1], ck);
			bootsAND(&temp[i], &temp[i], &Q2[i-2], ck);
			bootsXOR(&Q1[i], &Q2[i], &temp[i], ck);
		}
		else
			bootsCOPY(&Q1[i], &Q2[i], ck);
	}
	...
	for(int i = 0; i < size; i++) {
		if(i-4>=0) {
			bootsAND(&temp[i], &p[i], &p[i-1], ck);
			bootsAND(&temp[i], &temp[i], &p[i-2], ck);
			bootsAND(&temp[i], &temp[i], &p[i-3], ck);
			bootsAND(&temp[i], &temp[i], &Q1[i-4], ck);
			bootsXOR(&Q2[i], &Q1[i], &temp[i], ck);
		}
		else
			bootsCOPY(&Q2[i], &Q1[i], ck);
	...
	for(int i = 0; i < size; i++) {
		if(i>0)
      			bootsXOR(&sum[i], &p[i], &Q2[i-1], ck);
		else
			bootsCOPY(&sum[0], &Q2[0], ck);
    	}

  // clean up
  delete_gate_bootstrapping_ciphertext(Q1);
  delete_gate_bootstrapping_ciphertext(Q2);
  delete_gate_bootstrapping_ciphertext(p);
  delete_gate_bootstrapping_ciphertext(temp);

}

void Kogge_add(LweSample* sum, const LweSample* a, const LweSample* b, const TFheGateBootstrappingSecretKeySet *keyset,const TFheGateBootstrappingCloudKeySet* ck, const size_t size) {
    	const LweParams *in_out_params = keyset->params->in_out_params;
    	// carries
    	LweSample *Q1 = new_LweSample_array(size, in_out_params);
	LweSample *Q2 = new_LweSample_array(size, in_out_params);
	LweSample *p = new_LweSample_array(size, in_out_params);
	LweSample *temp = new_LweSample_array(size, in_out_params);
	LweSample *factor1 = new_LweSample_array(size, in_out_params);
	LweSample *factor2 = new_LweSample_array(size, in_out_params);
	...
	#pragma omp parallel for num_threads(NUM_THREADS)
  	for(int i = 0; i < size; i++) {
		...
		bootsAND(&Q1[i], &a[i], &b[i], ck);
		bootsXOR(&p[i], &a[i], &b[i], ck);
		bootsXOR(&factor1[i], &a[i], &b[i], ck);
		bootsXOR(&factor2[i], &a[i], &b[i], ck);
    	}
	...
	for(int j=1; j<=depth; j++) {
		if(j%2==1) {
			...
			for(int i = 0; i < size; i++) {
				if(i-h>=0) {
					bootsAND(&temp[i], &factor1[i], &Q1[i-h], ck);
					bootsXOR(&Q2[i], &Q1[i], &temp[i], ck);
					bootsAND(&factor2[i], &factor1[i], &factor1[i-h], ck);
				}
				else {
					bootsCOPY(&Q2[i], &Q1[i], ck);
				}
			...
			for(int i = 0; i < size; i++) {
				if(i-h>=0) {
					bootsAND(&temp[i], &factor2[i], &Q2[i-h], ck);
					bootsXOR(&Q1[i], &Q2[i], &temp[i], ck);
					bootsAND(&factor1[i], &factor2[i], &factor2[i-h], ck);
				}
				else {
					bootsCOPY(&Q1[i], &Q2[i], ck);
				}
	...
	if(depth%2==1) {
		...
		for(int i = 0; i < size; i++) {
			if(i>0)
      				bootsXOR(&sum[i], &p[i], &Q1[i-1], ck);
			else
				bootsCOPY(&sum[0], &Q1[0], ck);
    		}
	}
  ...
  delete_gate_bootstrapping_ciphertext(Q1);
  delete_gate_bootstrapping_ciphertext(Q2);
  delete_gate_bootstrapping_ciphertext(factor1);
  delete_gate_bootstrapping_ciphertext(factor2);
  delete_gate_bootstrapping_ciphertext(temp);
  delete_gate_bootstrapping_ciphertext(p);
